...
/* Derived from the record kind rather than stored: a pointer to a string
 * literal would be meaningless in the reading process. */
			     ...
			     char *out, uint32_t outsz)
{
	...
	/* Every trace record is a debug-level event by construction, so the
	 * level column is fixed; width matches the text log's. */
...
/*
 * Merged playback.  Both sources are already in time order on their own, so
 * this is a two-way merge that buffers neither file.
 */
...
struct trace_src {
	FILE    *f;
	char     rec[UNVME_TRACE_REC_MAX];
	uint32_t len;
	bool     valid;
	double   when;
};

/* "LEVEL   | YYYY-MM-DD HH:MM:SS.uuuuuu | ..." -> epoch seconds, or -1 for a
 * line with no parsable stamp, which is then emitted without being ordered
 * against the trace rather than held back. */
...
/* A short read means the writer is mid-record: rewind so the next call sees
 * the whole thing, rather than treating it as corruption. */
		    ...
		    bool nvme_only, bool follow)
{
	struct text_src  t = { 0 };
	struct trace_src s;
	char line[UNVME_TRACE_LINE_MAX];
	int  rc = 0;

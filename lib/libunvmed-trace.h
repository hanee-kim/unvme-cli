 ...
 *
 * Rendering a log line costs far more than capturing one, so these records
 * are written raw and rendered only when read back.  They live in their own
 * file so that unvmed.log stays text that cat and grep still work on.
 *
 * Everything here is plain data: records are read back by a different
 * process, so they may not contain pointers - not even to string literals,
 * whose addresses differ under ASLR.  The capture site is therefore derived
 * from @type rather than stored.
 */
...
/* Upper bound on one rendered line; the longest is a cmd_post with a fully
 * decoded command description. */
#define UNVME_TRACE_LINE_MAX	1024
/* Largest record a reader will accept; anything bigger means a corrupt
 * length field rather than a record from a newer writer. */
#define UNVME_TRACE_REC_MAX	4096

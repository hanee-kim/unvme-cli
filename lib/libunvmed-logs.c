...
/*
 * localtime_r() + strftime() cost more than the rest of a log line together
 * and their result only changes once a second, so cache it and rebuild on
 * rollover.  Per-thread, so there is no lock and no sharing.
 */
...
/*
 * These run on the I/O thread and only copy raw fields; libunvmed-trace.c
 * renders them later.  The reader is a different process, so no pointer can
 * be stored - hence the bdf is copied by value.
 */
...
/*
 * These four run once per submitted command and once per completion, so the
 * guard decides whether an fio run pays for capture it may then discard.
 *
 * #ifdef, not #ifndef: the records only exist when UNVME_DEBUG is set, so a
 * release build must not capture at all and a debug build must consult the
 * key.
 */

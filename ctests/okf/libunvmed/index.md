...
> **버전 스냅샷**: 이 문서들은 upstream 커밋 `e555bb7e976c96584a28ccffe12c72c5ba6ba597` 기준의 스냅샷이다. upstream이 갱신되면 각 문서 frontmatter의 `upstream_commit`과 최신 커밋을 비교해 재검토 대상을 선별할 것. 버전 추적 기록은 [`log.md`](log.md) 참고.

## 구조체 / 타입

| 문서 | 심볼 | 분류 |
|---|---|---|
| [struct-unvme.md](struct-unvme.md) | `struct unvme`, `enum unvme_state`, `unvmed_ctrl_get/set_state` | controller |
...
| [struct-unvme-vcq.md](struct-unvme-vcq.md) | `struct unvme_vcq`, `struct unvme_vcqe`, `unvmed_vcq_*` (`push_to_other` 포함) | io |
...
| [unvmed-ctrl-lifecycle.md](unvmed-ctrl-lifecycle.md) | `unvmed_init_ctrl`, `unvmed_get`/`unvmed_put`, `unvmed_free_ctrl[_all]`, `unvmed_ctrl_enabled`, `unvmed_get_epoch` | controller |
...
| [unvmed-reset.md](unvmed-reset.md) | `unvmed_reset_ctrl[_graceful]`, `unvme_reset_ctrl`, `unvmed_subsystem_reset`, `unvmed_flr`, `unvmed_hot_reset`, `unvmed_link_disable` | controller |
...
| 문서 | 심볼 | 분류 |
|---|---|---|
| [unvmed-io-queues.md](unvmed-io-queues.md) | `unvmed_create_sq/cq`, `unvmed_init_sq/cq[_iova]`, `unvmed_enable/disable_sq/cq`, `unvmed_free_sq/cq`, `unvmed_del_sq`, `unvmed_sq/cq_get/put/find`, `unvmed_get_sqs/cqs` | io |

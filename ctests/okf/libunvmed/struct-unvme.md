...
enum unvme_state {
	...
	UNVME_FATAL,		/* fatal status(CSTS.CFS=1) 발생, 더 이상 접근 불가 */
};
...
/* Return: 전이 성공 시 true, 실패 시 false + errno
 * (EALREADY: 이미 @state 상태, EINVAL: 현재 상태에서 허용되지 않는 전이) */
...
- `enum unvme_state`는 컨트롤러 인스턴스의 생명주기 상태(DISABLED → ENABLING → ENABLED → RESETTING → TEARDOWN, 그리고 fatal 시 FATAL)를 나타내며, `unvmed_state_str()`이 문자열 표현을 반환한다. `UNVME_FATAL`은 CSTS.CFS=1이 감지되어 컨트롤러에 더 이상 접근할 수 없는 상태다.
- 상태 조회/전이는 공개 함수 `unvmed_ctrl_get_state()`/`unvmed_ctrl_set_state()`로 수행한다. `set_state`는 허용되지 않는 전이면 `EINVAL`, 이미 해당 상태면 `EALREADY`를 errno에 설정하고 false를 반환한다.

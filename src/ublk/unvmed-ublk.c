...
retry:
	...
	/*
	 * -EAGAIN means the kernel deferred the command because it would
	 * have blocked in a non-blocking context (IO_URING_F_NONBLOCK).
			...
			struct ublksrv_ctrl_dev_info *info_out)
{
	...
	unvmed_log_info("ublk: ADD_DEV pid=%d nr_queues=%u depth=%u",
